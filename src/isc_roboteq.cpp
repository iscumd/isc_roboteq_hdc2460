...
namespace Roboteq
{
Roboteq::Roboteq(rclcpp::NodeOptions options) : Node("roboteq", options)
{
...
void Roboteq::connect()
{
  ...
  if (usb_port.empty())
  {
    RCLCPP_ERROR(this->get_logger(), "%s",
                 "Invalid or Empty Serial Port name:(");
    return;
  }

  RCLCPP_INFO(this->get_logger(), "%s%s", "The Set Port to open is ",
              usb_port.c_str());
...
void Roboteq::driveCallBack(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  move();
  RCLCPP_INFO(this->get_logger(), "Roboteq: %s%lf%s%lf",
              " Left Wheel = ", left_speed, " Right Wheel = ", right_speed);
}

bool Roboteq::send_Command(std::string command)
{
  BufferedFilterPtr echoFilter =
      serialListener.createBufferedFilter(SerialListener::exactly(command));
  serialPort.write(command + "\r");
  if (echoFilter->wait(50).empty())  // wait 50ms for a response
  {
    RCLCPP_ERROR(this->get_logger(), "%s",
                 "Failed to receive an echo from Roboteq:(");
    return false;
  }
  return true;
}
...
void Roboteq::move()
{
  ...
  {
    RCLCPP_ERROR(this->get_logger(), "%s",
                 "The Roboteq needs to connected first:(");
    return;
  }
  ...
  {
    send_Command(
        stringFormat("!a%.2X", std::clamp(right_speed, min_speed, max_speed)));
  }
  else
  {
    send_Command(
  ...
  {
    send_Command(
        stringFormat("!b%.2X", std::clamp(left_speed, min_speed, max_speed)));
  }
  else
  {
    send_Command(
